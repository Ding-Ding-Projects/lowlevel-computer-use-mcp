...
assert.match(main, /lowlevel-computer-use-cheap/);
assert.match(main, /legacyServerSpec/);
assert.doesNotMatch(main, /startup:install/);
assert.doesNotMatch(renderer, /installStartup/);
assert.match(html, /Local tool calls use the Cheap Version/);
...
assert.match(renderer, /window.__appearanceTargetSelfTest/);
assert.match(renderer, /normalizeAppearance/);
assert.match(renderer, /saveAppearancePreset/);
assert.match(renderer, /resetPopoverAppearance/);
assert.match(renderer, /appearanceTarget/);
...
for (const id of ['launchCommand', 'tabRegexPanel', 'commandPalette', 'confirmDialog', 'memory', 'memoryList', 'memoryLabel', 'fileTransferPanel', 'historyFilters', 'historyActionFilters', 'appearanceScale', 'appearancePreset', 'popoverFont', 'resetPopoverAppearance']) assert.match(renderer + html, new RegExp(id));
